...
client.on("messageCreate", (message) => {
  ...
  const isMentioned = message.mentions.users.has(targetId);
  const isTextMention = content.includes("kuromakii") || content.includes("@jamesliwis");

  // On vérifie une seule fois : si mention OU texte, mais on répond qu'une fois
  if (isMentioned || isTextMention) {
    const réponses = [
...
> _“Par Max Strobeni et James Liwis, que la vigne nous bénisse !”_`
    ...
    console.log(`[📢] Réponse envoyée à ${message.author.tag} dans ${message.guild?.name || "DM"}`);
    return;
  }
...
app.listen(8000, () =>
  console.log("✅ Fake web server for Koyeb started on port 8000")
);
